...
### User Stories
- As a Service Provider I want to have the ability to managed compute capacity dynamically via declarative API, so I can satisfy end user workloads compute demand:
  - Scaling.
  - Autoscaling.
  - Autorepair.
...
- As a cluster Admin I don't want to interact with Nodes and the underlying infra.
...
Its reason to exist is to preserve our ability to satisfy consumer needs and evolve at our own pace while reusing and relying on battle proven technologies for the implementation details.
...
When a HostedCluster is created a CAPI Cluster and a HostedControlPlane are created as a representation in the underneath the control plane namespace. This Cluster CR satisfies the contract for any CAPI Machine scalable resource.
...
- InPlace:
  - When a new release version is specified in a NodePool, this results in a new userdata created and updated in the MachineSet which enables upcoming Machines to come up with the targeted version. Existing Machines are upgrade in place (See section below).
...
- AutoRepair is delegated to CAPI by reconciling to create or updated opinionated MachineHealthCheck CRs.
- Bootstrapping CSR are handle by https://github.com/openshift/cluster-machine-approver.
...
- `MachineConfig` CRs can be embedded in ConfigMaps and associated to NodePools [via the `.config` the API know](https://github.com/openshift/hypershift/blob/main/api/v1alpha1/nodepool_types.go#L118-L127).
...
- After TTL/2 move the original token to rotated spot and create a new one. This results in both tokens for the same `.release` and `.config` pair coexisting during a TTL/2 duration. This give in flight operations e.g. Host booting with an userdata pointing to the rotated token.
...
#### Inplace Upgrader
The Inplace Upgrader is a controller that runs in the control plane namespace and watches both MachineSets (management cluster) and Nodes (guest cluster).
It orchestrates in place upgrades for a pool of Nodes i.e. MachineSets honouring the following design invariants:
...
- Prevent the cluster end user as much as possible from signaling back interfering in the product owned behaviour.
...
Workflow:
- The NodePool controller signal an upgrade (`.release` and `.config` pair change) annotation the underlying MachineSet.
...
- The Inplace Upgrader watches the Node annotation and move on appropriately, short-circuiting (in case of failure) or proceeding to next Node, until all Nodes are at the targeted version. 
